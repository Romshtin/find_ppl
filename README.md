...
## ⚖ Назначение и ограничения использования

Этот инструмент — **учебный разведывательный пайплайн**. Он собирает
**публично доступные** посты и комментарии из открычных Telegram-каналов
и VK-групп, фильтрует их по заранее заданным маркерам и складывает
в плоский JSON-архив для ручного разбора человеком.

**Что инструмент НЕ делает:**

- Не ломает авторизацию VK и не обходит лимиты user_token, кроме
  `wall.get` / `wall.getComments` для тех групп, в которые у пользователя
  и так есть доступ с открытым токеном.
- Не отправляет сообщения, не подписывается на каналы от имени
  пользователя, не модифицирует данные на стороне источника.
- Не хранит прокси-секреты, токены, сессии и архивы в репозитории —
  всё это в `.gitignore`.

**Ограничения, которые важно держать в голове перед использованием:**

- **ToS площадок.** Telegram и VK запрещают массовый сбор данных и
  автоматизацию без их одобрения. Собранные профили нельзя
  использовать для спама, таргетинга рекламы или передачи третьим
  лицам.
- **152-ФЗ и аналоги.** Пайплайн потенциально собирает
  идентифицируемые данные живых людей (имя, `from_id`, цитаты). Если
  архив хранится дольше, чем нужно для разовой разведки, обработка
  попадает под закон о персональных данных.
- **Этическая сторона.** Даже публичные посты — это высказывания
  конкретных людей без их согласия на профилирование по
  психологическим признакам. Перед личным контактом убедитесь, что
  это уместно и не нарушает чужих границ.
- **Юридическая ответственность — на пользователе.** Автор инструмента
  не несёт ответственности за то, как именно читатель использует
  собранные данные. Используйте пайплайн только в законных и
  этичных целях.

**Если вам нужен инструмент для продакшн-разведки, маркетинга или
сбора данных для ML** — этот репозиторий не для вас. Здесь
учебный/личный пайплайн с ограниченной пропускной способностью
(user_token VK живёт час, лимит 100 постов на группу за прогон, нет
параллелизма).
