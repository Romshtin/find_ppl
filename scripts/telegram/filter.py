"""Фильтр постов/комментариев Telegram-каналов по маркерам стратегии.
...
def filter_harvest(
    in_path: Path, out_path: Path, strategy: dict | None = None
) -> int:
    """Читает JSON одного чата, пишет только посты/комментарии с маркерами.
    Возвращает количество оставшихся постов (без учёта комментариев).
    """
...
def filter_all(strategy: dict | None = None) -> None:
    """Прогнать фильтр по raw/*.json Telegram-каналов — только каналы из стратегии.

    Если стратегия задаёт `channels`, берём только их (по stem имени файла).
    Иначе (legacy / нет стратегии) — обрабатываем все .json в RAW_DIR.
    """
    FILTERED_DIR.mkdir(parents=True, exist_ok=True)
    allowed: set[str] | None = None
    if strategy and "channels" in strategy:
        allowed = set(strategy["channels"])
